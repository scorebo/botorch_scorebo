...
def construct_script(args, cluster_oe_dir):
    ...
    if 'x_carhv' in os.getcwd():
        script.append(f"#SBATCH -A {args.project}")
        script.append(f'#SBATCH -n 6')
        script.append(f'#SBATCH --array 0-{num_tasks-1}')
        script.append(f'#SBATCH --mem-per-cpu 6000')
    else:
        script.append(f"#SBATCH --partition bosch_cpu-cascadelake")
        script.append(f"#SBATCH --mem 16G")
        script.append(f'#SBATCH --array 0-{num_tasks-1}%125')
        script.append(f'#SBATCH -c 4')
    ...
    script.append("")
    script.append(
        f"python benchmarking/learning_run.py "
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--project", default="snic2022-22-454")
    #parser.add_argument("--project", default="snic2022-5-250")
