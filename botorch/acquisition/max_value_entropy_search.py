...
class MaxValueBase(AcquisitionFunction, ABC):
    ...
    def __init__(
        ...
        input_max_values: Optional[Tensor] = None,
    ) -> None:
        ...
        self.is_batch = len(batch_shape) > 0
        ...
        if self.posterior_max_values.ndim == 2:
            self.posterior_max_values = self.posterior_max_values.unsqueeze(-1)
...
class qLowerBoundMaxValueEntropy(DiscreteMaxValueBase):
    ...
    def _compute_information_gain(
        ...
        prev_m = self.model.posterior(
            X.unsqueeze(-3), observation_noise=True
        )
        ...
        marg_mean = cond_means.mean(-3).unsqueeze(
            1).repeat(1, self.num_models, 1, 1)
        marg_variance = cond_variances.mean(-3).unsqueeze(
            1).repeat(1, self.num_models, 1, 1)
        ...
        mvs = self.posterior_max_values.permute(
        ...
        check_no_nans(ratio)
        covar_mM = covar_mM.unsqueeze(-1).repeat(1, 1, 1, self.num_samples)
        variance_M = variance_M.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 1, self.num_samples)
        ...
        if self.is_batch:
            acq = acq.mean(-1).squeeze(-1).unsqueeze(0)

        else:
            acq = acq.mean(dim=-1).unsqueeze(0).squeeze(-1).squeeze(-1)
