...
def uses_own_fit(model: Model) -> bool:
    """Check if a fully bayesian model uses slice sampling as opposed to NUTS.
    ...
    slice_bayesian_model_cls = (
        SliceFullyBayesianSingleTaskGP,
        PyroFullyBayesianSingleTaskGP,
    )
    
    return isinstance(model, slice_bayesian_model_cls)
