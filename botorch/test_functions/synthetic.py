...
class Forrester(SyntheticTestFunction):
    ...
    _optimal_value = 6.0208 # TODO check whether this is exact
    ...
    def __init__(
        ...
        self.dim = dim
        self._bounds = [(-0, 1)]
        self._optimizers = [(0.757285)] # TODO check whether this is exact
        super().__init__(noise_std=noise_std, negate=negate, bounds=bounds)

    def evaluate_true(self, X: Tensor) -> Tensor:
        return torch.sin(12 * X - 4) * torch.pow(6 * X - 2, 2)
