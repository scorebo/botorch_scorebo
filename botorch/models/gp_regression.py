...
class SingleTaskALGP(BatchedMultiOutputGPyTorchModel, ExactGP, FantasizeMixin):
    r"""A single-task exact GP model.
    ...
    def __init__(
        ...
        train_X, train_Y, _ = self._transform_tensor_args(X=train_X, Y=train_Y)
        if likelihood is None:
            noise_prior = LogNormalPrior(loc=0.0, scale=3.0 ** 0.5)
            likelihood = GaussianLikelihood(
                noise_prior=noise_prior,
                batch_shape=self._aug_batch_shape,
                noise_constraint=GreaterThan(
        ...
        if covar_module is None:
            covar_module = ScaleKernel(
                MaternKernel(
                    ...
                    ard_num_dims=transformed_X.shape[-1],
                    batch_shape=self._aug_batch_shape,
                    lengthscale_prior= LogNormalPrior(loc=0.0, scale=3.0 ** 0.5),
                ),
                batch_shape=self._aug_batch_shape,
                outputscale_prior= LogNormalPrior(loc=0.0, scale=3.0 ** 0.5),
            )
            self._subset_batch_dict = {
                "likelihood.noise_covar.raw_noise": -2,
                "mean_module.raw_constant": -1,
